...
describe('MSSQL advanced index DDL (SchemaDiff → SQL)', () => {
  test('Index INCLUDE and filtered (WHERE)', () => {
      ...
      tables: [
        {
          table: 'orders',
          indexCreates: [
            { name: 'idx_orders_user', columns: ['userId'], include: ['createdAt'], where: 'deleted = 0', unique: false }
          ]
        }
      ]
    };
    ...
    expect(text).toContain('CREATE INDEX [idx_orders_user] ON [orders] ([userId]) INCLUDE ([createdAt]) WHERE deleted = 0');
  });
});
