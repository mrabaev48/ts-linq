...
class Note { id!: number; title!: string; status!: 'draft' | 'published'; }

class MsCtx extends DbContext {
  constructor(url: string) { super({ provider: new MssqlProvider(url) as any }); }
}
...
describe('[integration][mssql] Conditional Validation with real DB', () => {
  ...
  beforeEach(() => {
    ...
    RequiredIfOf<Note>((n: Note) => n.status === 'published', 'Title required for published')(
      ...
      { kind: 'field', name: 'title', addInitializer: (fn: (this: unknown) => void) => fn.call(Note.prototype) } as any
    );
    ValidIfOf<Note>((n: Note) => n.status !== 'published' || (n.title || '').includes(' '), 'Title must contain space when published')(
      undefined as unknown as object,
      { kind: 'field', name: 'title', addInitializer: (fn: (this: unknown) => void) => fn.call(Note.prototype) } as any
    );
  });
  ...
  test('rejects invalid entity', async () => {
    ...
    const n = new Note(); n.status = 'published';
    set.add(n);
  ...
  test('inserts valid entity', async () => {
    ...
    await ctx.ensureCreated();
    const set = ctx.set(Note);
    const n = new Note(); n.status = 'published'; n.title = 'Hello World';
    set.add(n);
