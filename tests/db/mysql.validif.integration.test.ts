...
class Product { id!: number; title!: string; status!: 'draft' | 'published'; }

class MyCtx extends DbContext {
  constructor(url: string) { super({ provider: new MySqlProvider(url) as any }); }
}
...
describe('[integration][mysql] Conditional Validation with real DB', () => {
  ...
  beforeEach(() => {
    ...
    ValidIfOf<Product>((p: Product) => p.status !== 'published' || (p.title || '').includes(' '), 'Title must contain space when published')(
      undefined as unknown as object,
      { kind: 'field', name: 'title', addInitializer: (fn: (this: unknown) => void) => fn.call(Product.prototype) } as any
    );
  });
  ...
  test('rejects invalid entity', async () => {
    ...
    const p = new Product(); p.status = 'published';
    set.add(p);
  ...
  test('inserts valid entity', async () => {
    ...
    await ctx.ensureCreated();
    const set = ctx.set(Product);
    const p = new Product(); p.status = 'published'; p.title = 'Hello World';
    set.add(p);
