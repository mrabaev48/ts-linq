...
class T { id!: number; a!: number; b!: string }

describe('Dialect placeholders (property-based)', () => {
  beforeEach(() => {
    MetadataStorage.getInstance().clear();
    MetadataStorage.addEntity(T, 't');
    MetadataStorage.addColumn(T, { propertyName: 'id', columnName: 'id', type: 'INTEGER', nullable: false, isGenerated: true });
    MetadataStorage.addColumn(T, { propertyName: 'a', columnName: 'a', type: 'INTEGER', nullable: false });
    MetadataStorage.addColumn(T, { propertyName: 'b', columnName: 'b', type: 'TEXT', nullable: false });
  ...
  test('Postgres: ? -> $n in correct order; selectParams first', () => {
    fc.assert(
      fc.property(arbCounts, arbCounts, (selectCount, whereCount) => {
        ...
        const select = Array.from({ length: selectCount }, (_, i) => `jsonb_path_query_first(b, ?) AS j${i}`);
        const selectParams = Array.from({ length: selectCount }, (_, i) => `p${i}`);
        const where = Array.from({ length: whereCount }, (_, i) => ({ condition: `a > ?`, parameters: [i] }));
        const opts: QueryOptions = { select: selectCount ? select : ['id'], selectParams, where } as any;
  ...
  test('MySQL: ? preserved; count matches parameters', () => {
    fc.assert(
      fc.property(arbCounts, arbCounts, (selectCount, whereCount) => {
        ...
        const selectParams = Array.from({ length: selectCount }, (_, i) => `p${i}`);
        const where = Array.from({ length: whereCount }, () => ({ condition: `a > ?`, parameters: [1] }));
        const built = dialect.buildSelect(T as any, { select: selectCount ? select : ['id'], selectParams, where } as any);
        const total = selectCount + whereCount;
  ...
  test('SQLite: ? preserved; OFFSET without LIMIT inserts LIMIT -1', () => {
    ...
    const built = qb.generateSql(T as any, {
      select: ['id'],
