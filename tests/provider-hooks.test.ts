...
class HookedProvider extends SQLiteProvider {
    ...
    protected async beforeExecute(sql: string, params: any[]): Promise<void> { this.before.push(sql); }
    protected async afterExecute(sql: string, params: any[], result: any): Promise<void> { this.after.push(sql); }
}
