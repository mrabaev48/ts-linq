...
describe('CLI diff/verify PG e2e (conditional)', () => {
  const url = process.env.POSTGRES_URL || process.env.PG_URL;
  if (!url) {
    test.skip('skipped: POSTGRES_URL not set', () => {});
    return;
  }

  test('diff --json --details includes UNIQUE/CHECK for PG', () => {
...
class T {}
MetadataStorage.addEntity(T, 'T_cli_pg');
...
MetadataStorage.addIndex(T, { name: 'UQ_T_cli_pg_name', columns: ['name'], unique: true });
MetadataStorage.addCheck(T, { name: 'CK_name_len_pg', expression: 'char_length(name) > 0' });
`;
