...
describe('Performance optimizations', () => {
    ...
    test('Queryable.count uses optional cache with TTL', async () => {
        class Product { id!: number; name!: string; }
        // Register minimal metadata for Product
        MetadataStorage.addEntity(Product, 'Products');
        MetadataStorage.addPrimaryKey(Product, 'id');
        MetadataStorage.addColumn(Product, { propertyName: 'id', columnName: 'id', type: 'INTEGER', nullable: false, isGenerated: true });
        MetadataStorage.addColumn(Product, { propertyName: 'name', columnName: 'name', type: 'TEXT', nullable: false });
        ...
        class ProviderStub extends DatabaseProvider {
            ...
            public async beginTransaction(): Promise<void> { /* noop */ }
            public async commitTransaction(): Promise<void> { /* noop */ }
            public async rollbackTransaction(): Promise<void> { /* noop */ }
        }
