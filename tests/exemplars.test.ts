...
class ExHistogram {
  ...
  labels(lbl: any) { return { observe: (v: number, exemplar?: any) => this.calls.push({ lbl, v, exemplar }) }; }
}
class NoExHistogram {
  public calls: Array<{ lbl: any; v: number }> = [];
  labels(lbl: any) { return { observe: (v: number) => this.calls.push({ lbl, v }) }; }
}
class FakeCounter { labels(_lbl: any) { return { inc: (_v?: number) => {} }; } }
...
describe('PrometheusSqlLogger exemplars', () => {
  it('passes exemplar with traceId when histogram supports it', () => {
    ...
    logger.queryEnd?.({ sql: 'SELECT 1', params: [], durationMs: 7, provider: 'sqlite', traceId: 't-123' });
