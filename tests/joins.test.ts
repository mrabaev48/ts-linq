...
@Entity({ name: 'Authors' })
class Author { @PrimaryKey({ autoIncrement: true }) id!: number; @Column({ type: 'TEXT' }) name!: string; }

@Entity({ name: 'Books' })
class Book { @PrimaryKey({ autoIncrement: true }) id!: number; @Column({ type: 'TEXT' }) title!: string; @Column({ type: 'INTEGER' }) authorId!: number; }
...
class ProviderStub extends DatabaseProvider {
    ...
    protected async doExecuteQuery<T>(sql: string, params?: any[]): Promise<T[]> { this.lastSql = sql; return [] as any; }
    protected async doExecuteNonQuery(sql: string, params?: any[]): Promise<number> { this.lastSql = sql; return 0; }
