...
describe('FK add/drop SQL generation', () => {
  test('postgres add/drop fk', () => {
    const diff: SchemaDiff = {
      tables: [
        { table: 'orders', fkCreates: [{ name: 'fk_orders_users', columns: ['user_id'], refTable: 'users', refColumns: ['id'], onDelete: 'CASCADE' }] as any },
  ...
  test('mysql add/drop fk', () => {
    const diff: SchemaDiff = {
      tables: [
        { table: 'orders', fkCreates: [{ columns: ['user_id'], refTable: 'users', refColumns: ['id'] }] as any },
        { table: 'orders', fkDrops: ['fk_old'] as any }
      ]
    } as any;
