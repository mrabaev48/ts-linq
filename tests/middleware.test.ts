...
describe('OrmMiddleware pipeline', () => {
    ...
    const mw: OrmMiddleware = {
        beforeExecute: ({ sql, params }) => { beforeCalls.push({ sql, params }); },
        afterExecute: ({ sql, params, durationMs, rows }) => { afterCalls.push({ sql, params, durationMs, rows }); },
        entityMaterialized: ({ entity }) => { materialized.push(entity); }
    };
    ...
    beforeEach(async () => {
        ...
        beforeCalls.length = 0; afterCalls.length = 0; materialized.length = 0;
