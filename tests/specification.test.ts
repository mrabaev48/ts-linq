...
describe('Specification', () => {
    it('PredicateSpecification test() uses predicate', () => {
        const spec = new PredicateSpecification<any>(x => x.n > 5, null);
    ...
    it('Composite AND combines expressions', () => {
        ...
        const s1 = new PredicateSpecification<any>(x => x.a === 1, parser.parse((x:any)=>x.a===1));
        const s2 = new PredicateSpecification<any>(x => x.b > 0, parser.parse((x:any)=>x.b>0));
        const andSpec = Specs.and(s1, s2);
