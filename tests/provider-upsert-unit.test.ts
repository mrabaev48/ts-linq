...
@Entity({ name: 'UpUsers' })
class UpUser { @PrimaryKey({ autoIncrement: true }) id!: number; @Column({ type: 'TEXT', nullable: false }) name!: string; }
...
class FakePg extends PostgresProvider {
  ...
  protected async doExecuteQuery<T>(sql: string, params: any[] = []): Promise<T[]> { this.lastSql = sql; this.lastParams = params; return this.rowsToReturn as any; }
  protected async doExecuteNonQuery(sql: string, params: any[] = []): Promise<number> { this.lastSql = sql; this.lastParams = params; return 1; }
}
...
class FakeMy extends MySqlProvider {
  ...
  public async connect() { /* no-op */ }
  public async disconnect() { /* no-op */ }
  protected async doExecuteQuery<T>(sql: string, params: any[] = []): Promise<T[]> { this.lastSql = sql; this.lastParams = params; return [] as any; }
  protected async doExecuteNonQuery(sql: string, params: any[] = []): Promise<number> { this.lastSql = sql; this.lastParams = params; return 1; }
}
...
describe('Provider upsert SQL (unit)', () => {
  ...
  test('MySqlProvider generates ON DUPLICATE KEY UPDATE', async () => {
    ...
    const u = new UpUser(); u.id = 11 as any; u.name = 'A';
    await p.upsert(u, UpUser);
    expect(p.lastSql).toMatch(/INSERT INTO UpUsers/);
  ...
  test('MssqlProvider generates MERGE statement', async () => {
    ...
    const u = new UpUser(); u.id = 12 as any; u.name = 'A';
    await p.upsert(u, UpUser);
