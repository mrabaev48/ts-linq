...
describe('Diff generator: indexes and foreign keys (SQLite path)', () => {
  ...
  test('emits CREATE INDEX on create table', async () => {
    ...
    MetadataStorage.addColumn(A, { propertyName: 'id', columnName: 'id', type: 'INTEGER', nullable: false } as any);
    MetadataStorage.addPrimaryKey(A, 'id');
    // Secondary column with index
    MetadataStorage.addColumn(A, { propertyName: 'name', columnName: 'name', type: 'TEXT', nullable: true } as any);
    ...
    expect(steps.some((s) => /CREATE INDEX IF NOT EXISTS idx_A_name ON A \(name\)/.test(s.sql))).toBe(true);
  });
  ...
  test('emits DROP/CREATE INDEX for existing table diff', async () => {
    ...
    MetadataStorage.addColumn(T, { propertyName: 'x', columnName: 'x', type: 'INTEGER', nullable: true } as any);
    ...
    const provider = {
      ...
      executeQuery: async <T>(sql: string): Promise<T[]> => {
        if (sql.includes('sqlite_master') || sql.includes('PRAGMA table_info')) {
          ...
          if (sql.startsWith('PRAGMA')) return [{ name: 'id', type: 'INTEGER', notnull: 1, dflt_value: null, pk: 1 }] as unknown as T[];
    ...
    const gen = new DiffMigrationGenerator(provider);
    const steps = await gen.generate();
    expect(steps.some((s) => /CREATE INDEX IF NOT EXISTS idx_T_x ON T \(x\)/.test(s.sql))).toBe(true);
  });
  ...
  test('handles FK scenario for existing table (no-op for SQLite path)', async () => {
    ...
    MetadataStorage.addColumn(C, { propertyName: 'p_id', columnName: 'p_id', type: 'INTEGER', nullable: true } as any);
