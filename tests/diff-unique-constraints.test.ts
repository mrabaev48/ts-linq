...
describe('Diff generator: unique constraints', () => {
  ...
  test('PG: emits ADD CONSTRAINT UNIQUE on create and existing table', async () => {
    ...
    expect(
      stepsCreate.some((s) => /ALTER TABLE U ADD CONSTRAINT ux_U_a_b UNIQUE \(a, b\)/.test(s.sql))
    ).toBe(true);
    ...
    const providerExisting = {
      ...
      executeQuery: async <R>(sql: string): Promise<R[]> => {
        if (sql.includes('information_schema.tables'))
          return [{ table_name: 'U' }] as unknown as R[];
    ...
    expect(
      stepsExist.some((s) => /ALTER TABLE U ADD CONSTRAINT ux_U_a_b UNIQUE \(a, b\)/.test(s.sql))
    ).toBe(true);
  });
});
