...
function defineB() {
  @Entity()
  class BUser { @PrimaryKey({ autoIncrement: true }) id!: number; @Column({ type: 'TEXT', nullable: false }) name!: string; }
  return { BUser };
}
...
describe('Batch operations using base provider', () => {
  ...
  it('insertMany and updateMany work in a transaction', async () => {
    ...
    const users = Array.from({ length: 5 }, (_, i) => { const u = new BUser(); u.name = `U${i+1}`; return u; });
    ...
    const rows = await ctx.set(BUser).orderBy(x => x.id).toArray();
