...
describe('Integration Tests', () => {
    ...
    describe('Complete workflow', () => {
        it('should handle complete CRUD workflow', async () => {
            ...
            const userPosts = await context.set(Post).where(p => p.userId === user.id).toArray();
            ...
            const remainingPosts = await context.set(Post).where(p => p.userId === user.id).toArray();
    ...
    describe('Query operations', () => {
        ...
        it('should perform complex queries', async () => {
            ...
            const pagedUsers = await context.set(User)
                .orderBy(u => u.name)
                .skip(1)
                .take(2)
                .toArray();
            ...
            // Get first post
            const firstPost = await context.set(Post)
                .orderBy(p => p.id)
                .first();
