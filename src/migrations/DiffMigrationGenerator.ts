      ...
      if (td.columnChanges && td.columnChanges.length > 0) {
        ...
        for (const c of td.columnChanges) {
          if (c.kind === 'add') {
            ...
            const def =
              c.column.defaultValue !== undefined
                ? ` DEFAULT ${this.formatValue(c.column.defaultValue)}`
                : '';
            steps.push({
              sql: `ALTER TABLE ${td.table} ADD COLUMN ${c.column.name} ${this.mapType(c.column.type)}${nn}${def}`
            });
          }
        }
      }
