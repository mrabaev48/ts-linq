...
export class DiffMigrationGenerator {
    ...
    for (const tableDiff of diff.tables) {
      if (tableDiff.create) {
        ...
        for (const idx of tableDiff.create.indexes || []) {
          ...
          steps.push({
            sql: `CREATE ${uniq}INDEX IF NOT EXISTS ${idx.name} ON ${tableDiff.create.name} ${target}${where}`
          });
        }
        // Emit foreign keys (as follow-up constraints for broad compatibility)
        if (providerId !== 'sqlite')
          for (const fk of tableDiff.create.foreignKeys || []) {
            ...
            steps.push({
              sql: `ALTER TABLE ${tableDiff.create.name} ADD${name} FOREIGN KEY (${cols}) REFERENCES ${fk.refTable} (${refCols})${onDelete}${onUpdate}`
            });
          }
        ...
        if (providerId === 'sqlite') {
          for (const uq of tableDiff.create.uniqueConstraints || []) {
            ...
            steps.push({
              sql: `CREATE UNIQUE INDEX IF NOT EXISTS ${idxName} ON ${tableDiff.create.name} (${uq.columns.join(', ')})`
            });
          }
          ...
          for (const uq of tableDiff.create.uniqueConstraints || []) {
            ...
            steps.push({
              sql: `ALTER TABLE ${tableDiff.create.name} ADD CONSTRAINT ${uqName} UNIQUE (${uq.columns.join(', ')})`
            });
          }
          ...
          for (const ck of tableDiff.create.checkConstraints || []) {
            ...
            steps.push({
              sql: `ALTER TABLE ${tableDiff.create.name} ADD${ckName} CHECK (${ck.expression})`
            });
          }
        }
        continue;
      ...
      for (const idx of tableDiff.indexCreates || []) {
        ...
        const where = idx.where && providerId === 'postgresql' ? ` WHERE ${idx.where}` : '';
        steps.push({
          sql: `CREATE ${uniq}INDEX IF NOT EXISTS ${idx.name} ON ${tableDiff.table} ${target}${where}`
        });
      }
      ...
      if (providerId === 'sqlite') {
        ...
        for (const uq of tableDiff.uniqueCreates || []) {
          ...
          steps.push({
            sql: `CREATE UNIQUE INDEX IF NOT EXISTS ${idxName} ON ${tableDiff.table} (${uq.columns.join(', ')})`
          });
        }
        ...
        for (const ck of tableDiff.checkCreates || []) {
          steps.push({
            sql: `-- SQLite does not support ADD CHECK post-create (${ck.name || ''} ${ck.expression})`
          });
        }
      } else {
        for (const uqName of tableDiff.uniqueDrops || []) {
        ...
        for (const uq of tableDiff.uniqueCreates || []) {
          ...
          steps.push({
            sql: `ALTER TABLE ${tableDiff.table} ADD CONSTRAINT ${uqName} UNIQUE (${uq.columns.join(', ')})`
          });
        }
      ...
      for (const fk of tableDiff.fkCreates || []) {
        ...
        const onUpdate = fk.onUpdate ? ` ON UPDATE ${fk.onUpdate}` : '';
        steps.push({
          sql: `ALTER TABLE ${tableDiff.table} ADD${name} FOREIGN KEY (${cols}) REFERENCES ${fk.refTable} (${refCols})${onDelete}${onUpdate}`
        });
      }
      ...
      if (
        (tableDiff.columnChanges && tableDiff.columnChanges.length > 0) ||
        (providerId === 'sqlite' && hasFkMutation)
      ) {
        // If any alter/drop detected, rebuild table
        const hasDestructive =
          (tableDiff.columnChanges || []).some((change) => change.kind !== 'add') ||
          (providerId === 'sqlite' && hasFkMutation);
        if (hasDestructive) {
          ...
          steps.push({
            sql: this.buildCreateTableSql(
              ...
              cols,
              pkCols,
              providerId === 'sqlite' ? fks : undefined,
              undefined,
              providerId === 'sqlite' ? checks : undefined
            )
          });
          ...
          if (providerId === 'sqlite' && expected) {
            for (const uq of expected.uniqueConstraints || []) {
              ...
              steps.push({
                sql: `CREATE UNIQUE INDEX IF NOT EXISTS ${idxName} ON ${expected.name} (${uq.columns.join(', ')})`
              });
            }
            for (const idx of expected.indexes || []) {
              ...
              const target = idx.expression ? idx.expression : `(${idx.columns.join(', ')})`;
              steps.push({
                sql: `CREATE ${uniq}INDEX IF NOT EXISTS ${idx.name} ON ${expected.name} ${target}`
              });
            }
          }
          continue;
          ...
          if (!existing.has(col.columnName)) {
            ...
            const def =
              col.defaultValue !== undefined
                ? ` DEFAULT ${this.formatValue(col.defaultValue)}`
                : '';
    ...
    const expected: SchemaSnapshot = {
      tables: entities.map((entityMeta) => {
        ...
        const foreignKeys = (entityMeta.relationships || [])
          .filter((rel) =>
            Boolean(
              (rel as unknown as { foreignKey?: string; foreignKeys?: string[] }).foreignKey ||
                (rel as unknown as { foreignKeys?: string[] }).foreignKeys?.length
            )
          )
          .map((rel) => {
            const targetCtor = this.resolveTargetConstructor(
              rel.targetEntity as unknown as Function | (() => Function)
            );
            const targetMeta = MetadataStorage.getEntity(targetCtor);
            const refTable = targetMeta?.tableName || (targetCtor as Function).name;
            const refPkCols = (
              rel.referencedColumns && rel.referencedColumns.length > 0
            ...
            return {
              ...
              columns: (rel.foreignKeys && rel.foreignKeys.length > 0
                ? rel.foreignKeys
                : [rel.foreignKey as string]) as string[],
              refTable,
        ...
        return {
          ...
          checkConstraints: (entityMeta.checks || []).map((c) => ({
            name: c.name,
      ...
      for (const tableName of tableNames) {
        ...
        actualTables.push({
          ...
          columns: info.columns.map((c) => ({
            name: c.name,
            type: this.normalizeTypeByDialect(c.type, providerId),
            nullable: !c.notnull,
            defaultValue: (c as any).column_default
          })),
  ...
  private buildCreateTableSql(
    ...
    if (inlineFks && inlineFks.length > 0) {
      for (const fk of inlineFks) {
        ...
        const onUpdate = fk.onUpdate ? ` ON UPDATE ${fk.onUpdate}` : '';
        colDefs.push(
          `${name}FOREIGN KEY (${cols}) REFERENCES ${fk.refTable} (${refCols})${onDelete}${onUpdate}`
        );
      }
    }
  ...
  private mapTypeByDialect(type: string, providerId: string): string {
    ...
    switch (providerId) {
      case 'postgresql': {
        if (t === 'INTEGER' || t === 'INT' || t === 'BIGINT' || t === 'SMALLINT' || t === 'NUMBER')
          return 'INTEGER';
