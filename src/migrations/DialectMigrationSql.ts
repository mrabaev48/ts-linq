...
export function generateMigrationFromDiff(diff: SchemaDiff, dialect: Dialect): { up: string[]; down: string[] } {
    const up: string[] = [];
    const down: string[] = [];
        ...
        if (td.columnChanges && td.columnChanges.length > 0) {
            for (const ch of td.columnChanges) {
                if (ch.kind === 'add') {
                    ...
                    if (alterType) up.push(buildAlterTypeSql(dialect, td.table, ch.column.name, ch.column.type));
...
function buildCreateTableSql(td: TableDiff, dialect: Dialect): string {
    const t = td.create!;
    const cols = t.columns.map(c => `${q(dialect, c.name)} ${mapType(dialect, c.type)}${c.nullable ? '' : ' NOT NULL'}${c.defaultValue !== undefined ? ' DEFAULT ' + formatValue(dialect, c.defaultValue) : ''}`);
    if (t.primaryKeys && t.primaryKeys.length > 0) cols.push(`PRIMARY KEY (${t.primaryKeys.map(pk => q(dialect, pk)).join(', ')})`);
...
function buildAddColumnSql(dialect: Dialect, td: TableDiff, name: string, type: string, nullable: boolean, def?: any): string {
...
function buildAlterNullSql(dialect: Dialect, table: string, name: string, nullable: boolean): string {
    const t = q(dialect, table);
    const c = q(dialect, name);
    switch (dialect) {
        case 'postgresql':
...
function q(dialect: Dialect, id: string): string {
    switch (dialect) {
        case 'postgresql': return '"' + id + '"';
        case 'mysql': return '`' + id + '`';
        case 'mssql': return '[' + id + ']';
        default: return id;
    }
}
...
function formatValue(dialect: Dialect, v: any): string {
    ...
    if (typeof v === 'boolean') {
        switch (dialect) {
            ...
            case 'mysql': case 'sqlite': default: return v ? '1' : '0';
            case 'mssql': return v ? '1' : '0';
        }
    }
...
function norm(t: string): string { return String(t || '').trim().toUpperCase(); }
