 ...
 */
export function generateMigrationClassSource(diff: SchemaDiff, opts: MigrationTemplateOptions): string {
    ...
    const downBody = down.length > 0
        ? down.map(sql => `        await provider.executeNonQuery(` + '`' + esc(sql) + '`' + `);`).join('\n')
        : '        // no-op';
