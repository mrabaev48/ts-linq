...
export class MigrationFileBuilder {
  public static build(diff: SchemaDiff, opts: MigrationFileOptions): { filename: string; source: string } {
    const filename = `${opts.version}_${opts.className}.ts`;
    const { up, down } = generateMigrationFromDiff(diff, opts.dialect);
    const esc = (s: string) => s.replace(/`/g, '\\`');
    const upBody = up.map((sql) => `        await provider.executeNonQuery(` + '`' + esc(sql) + '`' + `);`).join('\n');
    const downBody = (down.length ? down : ['// no-op']).map((sql) => (sql.startsWith('//') ? `        ${sql}` : `        await provider.executeNonQuery(` + '`' + esc(sql) + '`' + `);`)).join('\n');
