...
export class MssqlProvider extends DatabaseProvider {
    ...
    private generateCreateTableSql(metadata: EntityMetadata): string {
        ...
        const columns: string[] = metadata.columns.map((c: ColumnMetadata) => this.generateColumnDefinition(c));
    ...
    private generateInsertSql(entity: any, metadata: EntityMetadata): { sql: string; params: any[]; returningPk?: string } {
