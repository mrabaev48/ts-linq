...
export class Queryable<T> {
    ...
    public where(predicate: (entity: T) => boolean): Queryable<T> {
        ...
        if (!parsed.parsed) {
            ...
            const whereClause: WhereClause = { condition: parsed.condition as any, parameters: parsed.parameters as any } as any;
    ...
    public take(count: number): Queryable<T> { this._options.limit = count; return this; }
    public skip(count: number): Queryable<T> { this._options.offset = count; return this; }
    public distinct(): Queryable<T> { this._options.distinct = true; return this; }
    ...
    public async toArray(): Promise<T[]> {
        const sql = this._sqlBuilder.generateSql(this._entityClass, this._options);
    ...
    public async first(): Promise<T> { this._options.limit = 1; const r = await this.toArray(); if (!r.length) throw new Error('Sequence contains no elements'); return r[0]; }
    public async firstOrDefault(): Promise<T | null> { this._options.limit = 1; const r = await this.toArray(); return r[0] ?? null; }
    ...
    public async count(): Promise<number> {
        ...
        if (this._options.where && this._options.where.length > 0) {
            const whereClauses = this._options.where.map(w => (w as any).condition);
            query += ` WHERE ${whereClauses.join(' AND ')}`;
            for (const where of this._options.where) parameters.push(...(where as any).parameters);
        }
        const results = await this._provider.executeQuery<{ count: number }>(query, parameters);
        return results[0].count;
    }
    public async any(): Promise<boolean> { this._options.limit = 1; const r = await this.toArray(); return r.length > 0; }
