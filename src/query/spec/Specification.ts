import { ExpressionNode, LogicalExpressionNode, LogicalOperator } from '../ast/Nodes';
...
export class CompositeSpecification<T> implements Specification<T> {
    ...
    toExpression(): ExpressionNode | null {
        ...
        const node: LogicalExpressionNode = { type: 'LogicalExpression', operator: this.operator, expressions: exprs };
        return node;
    }
    test(entity: T): boolean {
...
export class PredicateSpecification<T> implements Specification<T> {
    ...
    toExpression(): ExpressionNode | null { return this.expression; }
    test(entity: T): boolean { return this.predicate(entity); }
}

/** Helper factory with logical combinators. */
export const Specs = {
    and<T>(...specs: Array<Specification<T>>): Specification<T> { return new CompositeSpecification<T>(LogicalOperator.And, specs); },
    or<T>(...specs: Array<Specification<T>>): Specification<T> { return new CompositeSpecification<T>(LogicalOperator.Or, specs); }
};
