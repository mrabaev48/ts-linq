...
- **Triplicated private helpers.** `mapRowToEntity`/`coerceToSqlParameter` are copy-pasted across all three providers and already drifting; MSSQL alone coerces `undefined→null` (task-2).
...
| 4 | task-2 (shared mapper/coercer) | P1 | Removes triplication + `undefined` drift |
...
- `@ts-linq/core` (`DatabaseProvider` base, `SqlHelper`, geometry guards) — shared collaborators (task-2/3) likely land here.
