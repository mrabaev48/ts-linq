...
async function advancedQueriesExample() {
  ...
  try {
    ...
    const hasExpensiveProducts = await context.products.where((p) => p.price > 100).any();
    console.log(`Has expensive products (>$100): ${hasExpensiveProducts}`);

    // 4. Single and First operations
    console.log('\n4. Single/First operations:');

    const firstProduct = await context.products.orderBy((p) => p.price).first();
    console.log(`Cheapest product: ${firstProduct.name} - $${firstProduct.price}`);

    const mostExpensive = await context.products.orderByDescending((p) => p.price).first();
    ...
    const distinctProducts = await context.products.distinct().take(3).toArray();
