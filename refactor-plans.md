...
### Фаза 2 — SOLID Pass (3–5 дней) — [В РАБОТЕ]
- DIP: усилить абстракции вокруг провайдеров/диалектов/логгеров (внедрение зависимостей, минимизация знания о конкретных классах).
  ...
  - [NEXT] Логгеры/метрики: инъекция через интерфейс `SqlLogger` без знания конкретных реализаций.
  - [NEXT] Кэши (sqlGen/L2/count): инъекция стратегий/фабрик, ослабление связности с провайдером.
...
## Следующие действия
- Фаза 2: после завершённого DIP для диалектов — провести DIP для логгеров и кэшей, пройтись по ISP/SRP в модулях query/providers, подготовить почву для Фазы 3 (паттерны).
- Поддерживать временно ослабленные линтер-правила для тестов до завершения Фазы 2; затем выровнять с production-настройками.
