...
### Фаза 5 — Производительность (2–3 дня) — [ГОТОВО]
...
- [ГОТОВО] Метрики размеров/эвикций кэшей и TTL vs hard hits для count.
- [ГОТОВО] Безопасные вызовы метрик через хелпер (TSL_METRICS_DEBUG).
- [ГОТОВО] Внешние кэши: InMemorySqlCache и InMemoryCountCache (TTL+maxSize) + инъекция через PerformanceOptions.
- [ГОТОВО] Расширенный бенч (bench/multi.ts): SQLite/PG/MySQL по env, CSV/JSON вывод, сценарии include/offset vs keyset.
- [ГОТОВО] Профилировочные скрипты: bench:profile:cpu/heap.
- [ГОТОВО] Пример Grafana дашборда: docs/assets/grafana-dashboard.json.
- [ГОТОВО] Микро-оптимизации:
  - Queryable.executeCountQuery — сбор WHERE и параметров в один проход без лишних аллокаций.
  - QueryBuilder — лёгковесная сериализация ключа кэша вместо JSON.stringify.
- [ГОТОВО] Документация: раздел про внешние кэши и бенч/профилирование в README; Alerting (PromQL+Alertmanager) добавлен ранее.
- [ГОТОВО] Метрики размеров/эвикций кэшей, TTL vs hard hits, Gauge-привязки.
- [ГОТОВО] Профилирование (SQLite, CPU/Heap) + рекомендации по hot paths.
- [ГОТОВО] Аларминг и дашборд: PromQL, Alertmanager примеры, Grafana JSON.
