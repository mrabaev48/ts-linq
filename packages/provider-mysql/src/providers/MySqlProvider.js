...
const PoolAdapter_1 = require('../mysql/PoolAdapter');
const ErrorMapper_1 = require('../mysql/ErrorMapper');
class MySqlProvider extends core_1.DatabaseProvider {
  constructor(connectionString, logger, middlewares, softDelete, retryPolicy) {
    ...
    this.pool = null;
    this.ddl = new dialect_mysql_1.MySqlDdlStrategy();
    this.providerName = 'mysql';
  }
  async connect() {
    if (this.isConnected) return;
    this.pool = (0, PoolAdapter_1.createMySqlPool)(this.connectionString);
  ...
  async doExecuteQuery(sql, params = []) {
    try {
      ...
      throw (0, ErrorMapper_1.mapMySqlError)(e);
    }
  }
