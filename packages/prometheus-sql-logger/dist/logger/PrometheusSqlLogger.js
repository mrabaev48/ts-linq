...
exports.PrometheusSqlLogger = void 0;
class PrometheusSqlLogger {
    constructor(namespace, options) {
        ...
        if (this.client.Gauge) {
            ...
            this.activeTransactions = { inc: (labels, v) => { try {
                    gauge.inc(labels, v);
                }
                catch { } }, dec: (labels, v) => { try {
                    gauge.dec(labels, v);
                }
                catch { } } };
        }
    ...
    queryEnd(info) {
        ...
        try {
            ...
            try {
                this.queryDuration.labels(labels).observe(duration, info.traceId ? { traceId: info.traceId } : undefined);
            }
            catch {
                this.queryDuration.labels(labels).observe(duration);
            }
            if (info.error && this.errorTotal) {
                const errLabels = { provider, operation: op, entity, error_type: info.error.name || 'Error' };
    ...
    transactionStart(info) { if (this.enabled && this.activeTransactions) {
        try {
            this.activeTransactions.inc({ provider: info.provider || 'unknown' }, 1);
        }
        catch { }
    } }
    transactionEnd(info) { if (this.enabled && this.activeTransactions) {
        try {
    ...
    cacheEvicted(info) {
        if (!this.enabled || !this.cacheEvictions)
            return;
        try {
            this.cacheEvictions.labels({ cache: info.cache, provider: info.provider || 'unknown' }).inc(1);
        }
        catch { }
    }
    ...
    parseOperation(sql) { const m = sql.trim().match(/^(SELECT|INSERT|UPDATE|DELETE)\b/i); return m ? m[1].toUpperCase() : 'OTHER'; }
    parseEntity(sql) {
    ...
    cleanIdentifier(id) { return id.replace(/^["`\[]|["`\]]$/g, ''); }
}
exports.PrometheusSqlLogger = PrometheusSqlLogger;
