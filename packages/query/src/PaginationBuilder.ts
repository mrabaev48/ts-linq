...
import { type CteDefinition, type SqlParameter, ValidationError } from '@ts-linq/types';
...
export class PaginationBuilder<T> {
  ...
  async paginate(
    ...
    if (page < 1 || size < 1) {
      throw new ValidationError('paginate requires page >= 1 and size >= 1');
    }
  ...
  async keysetPaginate<TKey extends keyof T>(
    ...
    if (size < 1) throw new ValidationError('keysetPaginate requires size >= 1');
