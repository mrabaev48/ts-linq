...
const PoolAdapter_1 = require('../sqlite/PoolAdapter');
const ErrorMapper_1 = require('../sqlite/ErrorMapper');
class SQLiteProvider extends core_1.DatabaseProvider {
  constructor(connectionString, logger, middlewares, softDelete, retryPolicy) {
    ...
    this.db = null;
    this.ddl = new dialect_sqlite_1.SQLiteDdlStrategy();
    this.providerName = 'sqlite';
  }
  async connect() {
    this.db = (0, PoolAdapter_1.createSqliteDb)(this.connectionString);
  ...
  async insert(entity, entityClass) {
    ...
    if (primaryKey) {
      ...
      if (
        ...
        if (id !== undefined) entity[primaryKey] = id;
      }
    }
    return entity;
  }
  async update(entity, entityClass) {
    ...
    if (affectedRows === 0) {
      if (versionCol) throw new core_1.OptimisticConcurrencyError();
  ...
  async findById(id, entityClass) {
    ...
    const results = await this.executeQuery(sql, [this.coerceToSqlParameter(id)]);
  ...
  async doExecuteQuery(sql, params = []) {
    return new Promise((resolve, reject) => {
      ...
      this.db.all(sql, params, (err, rows) => {
        if (err) reject((0, ErrorMapper_1.mapSqliteError)(err));
        else resolve(rows);
      });
    });
  }
    ...
    if (metadata) {
      for (const column of metadata.columns) {
        if (Object.prototype.hasOwnProperty.call(row, column.columnName)) {
          entity[column.propertyName] = this.convertValueFromDatabase(
