...
class SQLiteProvider extends core_1.DatabaseProvider {
    ...
    async doExecuteQuery(sql, params = []) {
        return new Promise((resolve, reject) => {
            ...
            this.db.all(sql, params, (err, rows) => {
                if (err)
                    reject((0, ErrorMapper_1.mapSqliteError)(err));
                else
                    resolve(rows);
            });
        });
    ...
    getDialect() { return new dialect_sqlite_1.SQLiteDialect(); }
    ...
    mapTypeToSQLite(type) { return this.ddl.mapTypeToSQLite(type); }
