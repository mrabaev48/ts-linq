  ...
  test('insert then update same key via MERGE', async () => {
    ...
    try {
      await p.executeNonQuery(
        'IF OBJECT_ID(N"dbo.up_items", N"U") IS NOT NULL DROP TABLE [dbo].[up_items]'
      );
      await p.executeNonQuery(
        'CREATE TABLE [dbo].[up_items]([id] INT PRIMARY KEY, [name] NVARCHAR(100) NOT NULL)'
      );
      ...
      const rows = await p.executeQuery<{ name: string }>(
        'SELECT [name] FROM [dbo].[up_items] WHERE [id]=@p1',
