...
pgD('[integration][postgres] error mapping', () => {
  test('UNIQUE → UniqueConstraintError; FK → ForeignKeyConstraintError', async () => {
    ...
    try {
      ...
      await expect(p.executeNonQuery('INSERT INTO "parent"(u) VALUES($1)', ['x'] as unknown as never)).rejects.toBeInstanceOf(UniqueConstraintError);
      await expect(p.executeNonQuery('INSERT INTO "child"(pid) VALUES($1)', [9999] as unknown as never)).rejects.toBeInstanceOf(ForeignKeyConstraintError);
    } finally {
