...
(run ? describe : describe.skip)('DbSet + CountCache Integration', () => {
  ...
  it('should cache count() with predicate', async () => {
    ...
    const first = await users.query().where((user: User) => user.age >= 25).count();
    const callsAfterFirst = provider.executionCount;

    const second = await users.query().where((user: User) => user.age >= 25).count();
  ...
  it('should use different cache keys for different predicates', async () => {
    ...
    await users.query().where((user: User) => user.age >= 25).count();
    await users.query().where((user: User) => user.age >= 30).count();
  ...
  it('should invalidate count cache on entity insert', async () => {
    ...
    const newCount = await users.query().where((user: User) => user.age >= 25).count();
  ...
  it('should invalidate count cache on entity delete', async () => {
    ...
    const toDelete = await users.query().where((user: User) => user.name === 'Alice').first();
  ...
  it('should NOT invalidate count cache on entity UPDATE (count unchanged)', async () => {
    ...
    const bob = await users.query().where((user: User) => user.name === 'Bob').first();
    bob.name = 'Bobby';
    users.update(bob);
    await context.saveChanges();
    ...
    provider.nextResult = [{ count: 2 }];
    const count = await users.query().where((user: User) => user.age >= 25).count();
    expect(count).toBe(2);
  });
  ...
  it('should evict LRU entries when count cache full', async () => {
    ...
    await users.query().where((user: User) => user.age > 10).count(); // Key 1
    await users.query().where((user: User) => user.age > 20).count(); // Key 2
    
    expect(countCache.getSize()).toBe(2);
    
    await users.query().where((user: User) => user.age >= 25).count(); // Key 3, evicts Key 1
    ...
    // Key 1 again
    provider.nextResult = [{ count: 2 }];
    await users.query().where((user: User) => user.age > 10).count(); 
