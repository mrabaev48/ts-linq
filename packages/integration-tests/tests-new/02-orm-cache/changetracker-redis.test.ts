...
import { RedisEntityCacheAdapter, RedisClientLike } from '@ts-linq/cache-redis';
...
describe('ChangeTracker + Redis Cache Integration', () => {
  ...
  const serializer = (obj: unknown) => JSON.stringify(obj, (key, value) => {
    // ensure dates are serialized as ISO strings (standard JSON behavior)
    return value; 
  });
  
  const deserializer = (str: string) => JSON.parse(str, (key, value) => {
