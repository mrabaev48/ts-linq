...
// WKB codec tests are pure unit tests — no DB or PostGIS required.
describe('[unit] WKB codec round-trips', () => {
  test('WKB round-trip via encodeWkb / decodeWkb (Point)', () => {
...
pgDescribe('[integration][postgres] Spatial / PostGIS operations', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      // PostGIS extension not available in this environment — DB spatial tests will be skipped.
