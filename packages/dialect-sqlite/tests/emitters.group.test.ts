import { SQLiteGroupEmitter } from '../src/providers/sqlite/emitters/SQLiteGroupEmitter';
import type { QueryOptions, SqlParameter } from '@ts-linq/core';

test('group emitter renders GROUP BY and HAVING, collects params', () => {
  const emitter = new SQLiteGroupEmitter();
  const params: SqlParameter[] = [];
  const sql = emitter.emit(params, { groupBy: {
    columns: ['region'],
