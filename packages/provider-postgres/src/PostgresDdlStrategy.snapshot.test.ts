...
test('PG DDL snapshot: computed + defaults', () => {
  ...
  expect(ddl).toMatchInlineSnapshot(
    `"CREATE TABLE IF NOT EXISTS \"users_snap\" (\"id\" INTEGER NOT NULL, \"name\" TEXT NOT NULL, \"created_at\" TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP, \"name_len\" INTEGER GENERATED ALWAYS AS (length(name)) STORED, PRIMARY KEY (\"id\"))"`
  );
});
