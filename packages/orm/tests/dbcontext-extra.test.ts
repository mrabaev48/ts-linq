...
import { MetadataStorage, ValidationError } from '@ts-linq/core';
import type { DatabaseProvider } from '@ts-linq/core';
import { DbContext } from '@ts-linq/orm/src/context/DbContext';
import { LoadingStrategy } from '@ts-linq/orm/src/loading/LoadingStrategy';

// Локально заглушим clearCountCache через require мока по alias, чтобы не падать на резолве пути
try {
  jest.mock('@ts-linq/core/src/query/Queryable', () => ({
    Queryable: { clearCountCache: jest.fn() }
  }));
} catch {}
try {
  jest.mock('@ts-linq/core/query/Queryable', () => ({
    Queryable: { clearCountCache: jest.fn() }
  }));
} catch {}
...
describe('DbContext extra coverage', () => {
  ...
  test('include: non-proxy uses EntityLoader.populateRelationships', async () => {
    ...
    const ctx = new TestContext({ provider });
    const spy = jest
      .spyOn(require('@ts-linq/orm').EntityLoader.prototype, 'populateRelationships')
  ...
  test('include: lazy proxy uses LazyLoadingProxy.preloadRelationships', async () => {
    const provider = providerStub();
    const ctx = new TestContext({ provider });
    const lp = require('@ts-linq/orm').LazyLoadingProxy as {
  ...
  test('commitTransaction and rollbackTransaction clear caches and call provider', async () => {
    ...
    const { Queryable } =
      require('@ts-linq/core/src/query/Queryable') || require('@ts-linq/core/query/Queryable');
