...
test('SQLite DDL snapshot: computed VIRTUAL + defaults', () => {
  ...
  expect(ddl).toMatchInlineSnapshot(`"CREATE TABLE IF NOT EXISTS users_snap (id INTEGER NOT NULL, a INTEGER NOT NULL, doubleA GENERATED ALWAYS AS (a * 2) VIRTUAL, PRIMARY KEY (id))"`);
});
