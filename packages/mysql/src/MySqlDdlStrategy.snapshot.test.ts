...
class SnapUser { id!: number; a!: number; doubleA!: number }

test('MySQL DDL snapshot: computed VIRTUAL + defaults', () => {
  ...
  const cols: ColumnMetadata[] = [
    ...
    { propertyName: 'doubleA', columnName: 'doubleA', type: 'INTEGER', nullable: true, isComputed: true, computedExpression: 'a * 2' }
  ];
  ...
  expect(ddl).toMatchInlineSnapshot(`"CREATE TABLE IF NOT EXISTS users_snap (id INT NOT NULL, a INT NOT NULL, doubleA INT GENERATED ALWAYS AS (a * 2) VIRTUAL, PRIMARY KEY (id))"`);
});
