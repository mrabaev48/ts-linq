...
test('MSSQL DDL snapshot: computed PERSISTED', () => {
  ...
  expect(ddl).toMatchInlineSnapshot(`"IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'users_snap') BEGIN CREATE TABLE users_snap (id INT NOT NULL, a INT NOT NULL, doubleA AS (a * 2), PRIMARY KEY (id)) END"`);
});
