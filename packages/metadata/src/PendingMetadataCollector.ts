...
class PendingMetadataCollectorImpl {
  private columns = new WeakMap<EntityCtor, Map<string, ColumnMetadata>>();
  private primaryKeys = new WeakMap<EntityCtor, Set<string>>();
  private indexes = new WeakMap<EntityCtor, IndexMetadata[]>();
  private relationships = new WeakMap<EntityCtor, Map<string, RelationshipMetadata>>();
  ...
  getColumns(target: EntityCtor): Map<string, ColumnMetadata> {
  ...
  getPrimaryKeys(target: EntityCtor): Set<string> {
  ...
  getIndexes(target: EntityCtor): IndexMetadata[] {
  ...
  getRelationships(target: EntityCtor): Map<string, RelationshipMetadata> {
