    ...
    for (const nameRaw of id) {
        switch (dialect) {
            case 'postgresql':
                ...
                break;
            case 'mysql':
                up.push(`ALTER TABLE ${(0, SqlUtils_1.q)(dialect, td.table)} DROP INDEX ${(0, SqlUtils_1.q)(dialect, nameRaw)}`);
                break;
            case 'mssql':
                up.push(`DROP INDEX ${(0, SqlUtils_1.q)(dialect, nameRaw)} ON ${(0, SqlUtils_1.q)(dialect, td.table)}`);
                break;
            default:
                up.push(`DROP INDEX IF EXISTS ${(0, SqlUtils_1.q)(dialect, nameRaw)}`);
        }
