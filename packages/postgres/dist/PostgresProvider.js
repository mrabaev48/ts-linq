...
class PostgresProvider extends core_1.DatabaseProvider {
  ...
  async insert(entity, entityClass) {
    ...
    const values = cols.map((c) =>
      this.coerceToSqlParameter(convertValueForPg(entity[c.propertyName], c.type))
    );
  ...
  async upsert(entity, entityClass) {
    ...
    const rows = await this.executeQuery(sql, values);
    if (rows[0]) Object.assign(entity, rows[0]);
    return entity;
  }
  ...
  async findById(id, entityClass) {
    ...
    const rows = await this.executeQuery(sql, [this.coerceToSqlParameter(id)]);
...
function mapPgError(err) {
  ...
  if (code === '23505') return new core_1.UniqueConstraintError(message, code);
  if (code === '23503') return new core_1.ForeignKeyConstraintError(message, code);
