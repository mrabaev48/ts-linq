...
class Queryable {
  ...
  constructor(entityClass, provider, entityLoader, entityCache, performance, globalFilters) {
    ...
    this._sqlBuilder = new QueryBuilder_1.QueryBuilder(
      provider.getDialect(),
      provider.loggerRef,
      provider.providerLabel,
      performance?.sqlCache
    );
    this._materializer = new RowMaterializer_1.RowMaterializer(
      this._entityClass,
      this._provider,
      this._entityCache,
      this._performance
    );
    this._includePlanner = new IncludePlanner_1.IncludePlanner(
      this._entityLoader,
      this._entityClass
    );
  }
  ...
  clone() {
    const clonedQueryable = new Queryable(
      this._entityClass,
      this._provider,
      this._entityLoader,
      this._entityCache,
  ...
  tryGetFromCache(row, metadata) {
    ...
    const idValue = pkCol ? row[pkCol.columnName] : row[pkProp];
  ...
  notifyMaterialized(entity, metadata) {
    try {
      if (metadata) this._provider.notifyEntityMaterialized?.(entity, metadata);
    } catch {
      // ignore debug metric errors
    }
  }
  ...
  parseJoinPredicate(onStr, leftTable, rightTable, leftMeta, rightMeta) {
    return JoinPredicateParser_1.JoinPredicateParser.parse(
      onStr,
      leftTable,
      rightTable,
      leftMeta,
