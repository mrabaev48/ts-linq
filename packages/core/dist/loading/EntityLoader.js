...
class EntityLoader {
  ...
  async loadEntity(entityClass, id, options) {
    ...
    if (
      loadingOptions.strategy === LoadingStrategy_1.LoadingStrategy.Eager ||
  ...
  async loadToOne(entity, relationship, targetCtor, nextOptions) {
    ...
    if (relatedEntity) entity[relationship.propertyName] = relatedEntity;
  }
  ...
  async loadRelationshipBatchedByType(
    entities,
    entityClass,
    metadata,
    relationship,
  ...
  async loadToOneBatched(entities, meta, relationship, targetCtor, options, depth) {
    ...
    for (const relatedEntity of related) byId.set(relatedEntity[targetPk], relatedEntity);
    for (const entityItem of entities) {
      const fk = entityItem[foreignKeyName];
      if (fk !== undefined && fk !== null) entityItem[relationship.propertyName] = byId.get(fk);
    }
    if (depth - 1 > 0)
