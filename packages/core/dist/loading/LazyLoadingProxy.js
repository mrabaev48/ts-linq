...
exports.LazyLoadingProxy =
  exports.LAZY_LOADING_STATE =
  exports.LAZY_LOADING_PROXY =
  exports.LAZY_LOADING_PROVIDER =
  exports.LAZY_LOADING_TARGET =
    void 0;
...
class LazyLoadingProxy {
  ...
  static proxyGet(target, prop, receiver, provider, entityClass, metadata, state) {
    if (prop === exports.LAZY_LOADING_TARGET) return target;
    if (prop === exports.LAZY_LOADING_PROVIDER) return provider;
    if (prop === exports.LAZY_LOADING_PROXY) return true;
    if (prop === exports.LAZY_LOADING_STATE) return state;
  ...
  static proxyHas(target, prop) {
    if (
      prop === exports.LAZY_LOADING_PROXY ||
  ...
  static proxyOwnKeys(target) {
    ...
    return keys.filter(
      (key) =>
        key !== exports.LAZY_LOADING_TARGET &&
  ...
  static async loadManyToMany(entity, entityClass, metadata, relationship, targetCtor, provider) {
    ...
    const targetPk = (MetadataStorage_1.MetadataStorage.getEntity(targetCtor)?.primaryKeys ||
      [])[0];
  ...
  static getColumnNameForPk(targetCtor, targetPk) {
    return (
      (MetadataStorage_1.MetadataStorage.getEntity(targetCtor)?.columns || []).find(
        (c) => c.propertyName === targetPk
      )?.columnName || targetPk
    );
  }
  static async batchLoadManyToMany(
    entities,
    entityClass,
    relationship,
    provider,
  ...
  static extractSourceIds(entities, sourcePk) {
    const ids = entities.map((e) => e[sourcePk]).filter((v) => v !== undefined && v !== null);
  ...
  static assignManyToManyCollections(entities, propName, sourcePk, bySource, relById) {
    for (const entity of entities) {
      ...
      entity[propName] = idList.map((id) => relById.get(id)).filter(Boolean);
      const state = this.getLoadingState(entity);
      if (state) this.markLoaded(state, propName);
    }
  }
