...
class DbContext {
  ...
  constructor(options) {
    ...
    try {
      this._provider.softDelete = options.softDelete;
    } catch {
      /* ignore */
    }
    ...
    this._validationService = new ChangeValidationService_1.ChangeValidationService(
    ...
    this._insertCmd = new InsertCommand_1.InsertCommand(this._provider, (c) =>
      this.updateEntityCache(c)
    );
    this._updateCmd = new UpdateCommand_1.UpdateCommand(this._provider, (c) =>
      this.updateEntityCache(c)
    );
    this._deleteCmd = new DeleteCommand_1.DeleteCommand(
      this._provider,
  ...
  async include(entity, entityClass, ...propertyNames) {
    if (LazyLoadingProxy_1.LazyLoadingProxy.isLazyProxy(entity)) {
      ...
      await LazyLoadingProxy_1.LazyLoadingProxy.preloadRelationships(
        [entity],
        entityClass,
        propertyNames,
        this._provider
  ...
  validateChanges(changes) {
    ...
    if (errors.length > 0) throw new types_1.ValidationError('Model validation failed', errors);
  }
