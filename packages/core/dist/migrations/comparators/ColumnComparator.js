...
exports.isColumnAltered = isColumnAltered;
exports.diffColumns = diffColumns;
function normalizeType(typeName) {
    return String(typeName || '')
        .trim()
        .toUpperCase();
}
function isColumnAltered(expectedColumn, actualColumn) {
    ...
    const defaultExprChanged = (expectedColumn.defaultExpression || '') !== (actualColumn.defaultExpression || '');
    return typeChanged || nullableChanged || computedChanged || defaultExprChanged;
}
function diffColumns(expectedTable, actualTable) {
    const changes = [];
