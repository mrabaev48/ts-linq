...
function handleTableRename(td, dialect, up) {
  if (td.renameTo) {
    ...
    switch (dialect) {
      case 'postgresql':
        up.push(
          `ALTER TABLE ${(0, SqlUtils_1.q)(dialect, td.table)} RENAME TO ${(0, SqlUtils_1.q)(dialect, to)}`
        );
        break;
      case 'mysql':
        up.push(
          `RENAME TABLE ${(0, SqlUtils_1.q)(dialect, td.table)} TO ${(0, SqlUtils_1.q)(dialect, to)}`
        );
        break;
      case 'mssql':
...
function handleCreateTable(td, dialect, up, down) {
  ...
  if (td.create.indexes && td.create.indexes.length > 0) {
    for (const idx of td.create.indexes) {
      ...
      up.push(
        `CREATE ${uniq}INDEX ${name} ON ${(0, SqlUtils_1.q)(dialect, td.create.name)} (${cols})${where}`
      );
    }
  }
...
function buildCreateTableSql(td, dialect) {
  ...
  if (create.primaryKeys && create.primaryKeys.length > 0)
    cols.push(
      `PRIMARY KEY (${create.primaryKeys.map((pk) => (0, SqlUtils_1.q)(dialect, pk)).join(', ')})`
    );
