...
class MigrationRunner {
    ...
    async getAppliedMigrations() {
        try {
            const results = await this._provider.executeQuery('SELECT version, name, applied_at FROM __migrations ORDER BY version');
        ...
        for (const migration of this._migrations) {
            if (!appliedVersions.has(migration.getVersion())) {
                ...
                try {
                    ...
                    await migration.up();
                    await this._provider.executeNonQuery('INSERT INTO __migrations (version, name, applied_at) VALUES (?, ?, ?)', [migration.getVersion(), migration.getName(), new Date().toISOString()]);
