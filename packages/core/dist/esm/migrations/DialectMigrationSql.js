export function generateMigrationFromDiff(diff, dialect) {
  ...
  for (const tableDiff of diff.tables) {
    ...
    if (tableDiff.create) {
      ...
      if (tableDiff.create.indexes && tableDiff.create.indexes.length > 0) {
        for (const idx of tableDiff.create.indexes) {
          ...
          up.push(
            `CREATE ${uniq}INDEX ${name} ON ${q(dialect, tableDiff.create.name)} (${cols})${where}`
          );
        }
      }
    ...
    if (tableDiff.indexCreates && tableDiff.indexCreates.length > 0) {
      for (const idx of tableDiff.indexCreates) {
        ...
        const where = idx.where && dialect !== 'mysql' ? ` WHERE ${idx.where}` : '';
        const using =
          dialect === 'postgresql' && idx.using ? ` USING ${idx.using.toUpperCase()}` : '';
        const concurrently = dialect === 'postgresql' && idx.concurrently ? ' CONCURRENTLY' : '';
        const withSql =
          dialect === 'postgresql' && idx.withParams && Object.keys(idx.withParams).length > 0
            ? ` WITH (${Object.entries(idx.withParams)
                .map(([k, v]) => `${k}=${typeof v === 'string' ? `'${v}'` : String(v)}`)
                .join(', ')})`
            : '';
        const visibility =
          dialect === 'mysql' && idx.mysqlVisibility ? ` ${idx.mysqlVisibility}` : '';
        const include =
          dialect === 'mssql' && idx.include && idx.include.length > 0
            ? ` INCLUDE (${idx.include.map((c) => q(dialect, c)).join(', ')})`
            : '';
        switch (dialect) {
          case 'postgresql':
            up.push(
              `CREATE ${uniq}INDEX${concurrently} ${name} ON ${q(dialect, tableDiff.table)}${using} (${cols})${withSql}${where}`
            );
            break;
          case 'mysql':
            up.push(
              `CREATE ${uniq}INDEX ${name} ON ${q(dialect, tableDiff.table)} (${cols})${visibility}`
            );
            break;
          case 'mssql':
            up.push(
              `CREATE ${uniq}INDEX ${name} ON ${q(dialect, tableDiff.table)} (${cols})${include}${where}`
            );
            break;
          default:
            up.push(
              `CREATE ${uniq}INDEX ${name} ON ${q(dialect, tableDiff.table)} (${cols})${where}`
            );
        }
      }
    }
    if (tableDiff.indexDrops && tableDiff.indexDrops.length > 0) {
    ...
    if (tableDiff.fkCreates && tableDiff.fkCreates.length > 0) {
      for (const fk of tableDiff.fkCreates) {
        ...
        switch (dialect) {
          ...
          case 'mssql':
            up.push(
              `ALTER TABLE ${q(dialect, tableDiff.table)} ADD ${name}FOREIGN KEY (${cols}) REFERENCES ${q(dialect, fk.refTable)} (${refCols})${onDel}${onUpd}`
            );
            break;
          default:
            up.push(
    ...
    if (tableDiff.fkDrops && tableDiff.fkDrops.length > 0) {
      ...
      for (const ch of tableDiff.columnChanges) {
        if (ch.kind === 'add') {
          ...
          const computedChanged =
            ch.prev?.isComputed !== ch.column.isComputed ||
    ...
    if (tableDiff.columnRenames && tableDiff.columnRenames.length > 0) {
