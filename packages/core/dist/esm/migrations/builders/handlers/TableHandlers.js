...
export function handleTableRename(td, dialect, up) {
    if (td.renameTo) {
...
export function handleCreateTable(td, dialect, up, down) {
    if (!td.create)
        return false;
...
export function handleDropTable(td, dialect, up) {
    if (!td.drop)
        return false;
    up.push(`DROP TABLE ${q(dialect, td.table)}`);
    return true;
}
export function buildCreateTableSql(td, dialect) {
    ...
    if (create.foreignKeys && create.foreignKeys.length > 0) {
        for (const fk of create.foreignKeys)
            cols.push(buildInlineFkSql(dialect, fk));
    }
