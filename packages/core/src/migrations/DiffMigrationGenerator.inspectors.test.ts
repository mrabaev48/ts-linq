...
class FakeProvider extends DatabaseProvider {
  constructor(public label: 'postgresql' | 'mysql' | 'mssql', private responses: Array<{ like: RegExp; rows: unknown[] }>) {
    super('');
...
function registerUsers(withIndex: { name: string; columns: string[]; unique: boolean; where?: string }) {
  class User {}
  MetadataStorage.getInstance().clear();
  MetadataStorage.addEntity(User, 'Users');
  const cols: ColumnMetadata[] = [
    { propertyName: 'id', columnName: 'id', type: 'INTEGER', nullable: false } as unknown as ColumnMetadata,
    { propertyName: 'active', columnName: 'active', type: 'BOOLEAN', nullable: false } as unknown as ColumnMetadata,
    { propertyName: 'email', columnName: 'email', type: 'TEXT', nullable: false } as unknown as ColumnMetadata,
  ];
  cols.forEach(c => MetadataStorage.addColumn(User, c));
  MetadataStorage.addPrimaryKey(User, 'id');
  MetadataStorage.addIndex(User, { name: withIndex.name, columns: withIndex.columns, unique: withIndex.unique, where: withIndex.where } as any);
}

test('DiffMigrationGenerator uses Postgres inspector to create missing filtered index', async () => {
  registerUsers({ name: 'idx_users_active', columns: ['active'], unique: false, where: 'active = true' });
...
test('DiffMigrationGenerator uses MySQL inspector to create missing index (ignores WHERE)', async () => {
  registerUsers({ name: 'idx_users_active', columns: ['active'], unique: false, where: 'active = 1' });
