...
export class MySqlSchemaInspector {
  ...
  public async getIndexes(table: string): Promise<TableIndexDef[]> {
    const rows = await this.provider.executeQuery<{ INDEX_NAME: string; NON_UNIQUE: 0 | 1; COLUMN_NAME: string | null; SEQ_IN_INDEX: number; COLLATION?: 'A' | 'D' | null; EXPRESSION?: string | null }>(
    ...
    const byName = new Map<string, TableIndexDef & { _parts: Array<{ col?: string; expr?: string }> }>();
    for (const r of rows) {
      const name = r.INDEX_NAME;
      if (!byName.has(name)) byName.set(name, { name, columns: [], unique: r.NON_UNIQUE === 0, _parts: [] });
...
export class MssqlSchemaInspector {
  ...
  public async listTables(): Promise<string[]> {
    const rows = await this.provider.executeQuery<{ name: string }>('SELECT name FROM sys.tables ORDER BY name');
    return rows.map((r) => r.name);
  }

  public async getIndexes(table: string): Promise<TableIndexDef[]> {
    const idxRows = await this.provider.executeQuery<{ name: string; is_unique: 0 | 1; filter_definition: string | null }>(
    ...
    const colRows = await this.provider.executeQuery<{ index_name: string; column_name: string; key_ordinal: number; is_descending_key: 0 | 1 }>(
