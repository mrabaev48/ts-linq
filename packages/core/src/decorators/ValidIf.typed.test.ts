...
import { ValidIfOf, RequiredIfOf, MinLengthOf, MaxLengthOf, PatternOf, RangeOf, ValidIf } from './ValidIf';

type Stage3FieldContext = { kind: 'field'; name: string | symbol; addInitializer: (fn: (this: unknown) => void) => void };
...
describe('Typed ValidIf helpers', () => {
  beforeEach(() => {
    ...
    (RequiredIfOf<Product>(p => p.status === 'published') as unknown as Stage3FieldDecorator)(
      ...
      { kind: 'field', name: 'title', addInitializer: (fn) => fn.call(Product.prototype) } as Stage3FieldContext
    );
    // 2) RangeOf for price (>= 0)
    (RangeOf<Product>(0) as unknown as Stage3FieldDecorator)(
      undefined as unknown as object,
      { kind: 'field', name: 'price', addInitializer: (fn) => fn.call(Product.prototype) } as Stage3FieldContext
    );
  });
  ...
  test('ValidIf supports phases and i18n translation', async () => {
    class TCtx extends Ctx {
      constructor() {
        super();
        (this as unknown as { _validationOptions?: { translate?: (k: string) => string } })._validationOptions = {
    ...
    (ValidIf((e) => (e as Product).status !== 'published' || !!(e as Product).title, undefined, { phase: 'onCreate', messageKey: 'err.title.required' }) as unknown as Stage3FieldDecorator)(
      undefined as unknown as object,
      { kind: 'field', name: 'title', addInitializer: (fn) => fn.call(Product.prototype) } as Stage3FieldContext
    );
    const p = new Product(); p.status = 'published'; p.price = 10;
    ...
    try {
      ...
      const details: Array<{ message: string }> = (e as { details?: Array<{ message: string }> } | undefined)?.details || [];
