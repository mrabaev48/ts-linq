...
describe('Multiple ValidIf rules', () => {
  beforeEach(() => {
    ...
    MinLengthOf<User>(3, 'Name too short')(
      ...
      { kind: 'field', name: 'name', addInitializer: (fn: (this: unknown) => void) => fn.call(User.prototype) } as any
    );
  ...
  test('aggregates multiple errors for the same property', async () => {
    const ctx = new Ctx(); const set = ctx.set(User);
    const u = new User(); u.email = 'a@b'; u.name = ''; // violates both rules
    set.add(u);
    try {
      ...
      const messages = (ve.details || []).filter(d => d.property === 'name').map(d => d.message);
