...
describe('IndexOptionsBuilder', () => {
    ...
    test('throws when name is empty', () => {
        expect(() => new IndexOptionsBuilder_1.IndexOptionsBuilder(' ').onColumns(['a']).build()).toThrow('Index name must be provided');
    });
    test('throws when no columns provided', () => {
        expect(() => new IndexOptionsBuilder_1.IndexOptionsBuilder('idx').build()).toThrow('Index must reference at least one column');
    });
});
