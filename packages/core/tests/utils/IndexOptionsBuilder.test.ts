...
describe('IndexOptionsBuilder', () => {
  ...
  test('merges orderBy, expressions, collations and nulls', () => {
    const opts = new IndexOptionsBuilder('idx_complex')
  ...
  test('supports Postgres-specific options', () => {
    const opts = new IndexOptionsBuilder('idx_pg')
      .onColumns(['title'])
      .using('gin')
      .concurrently(true)
  ...
  test('supports MySQL visibility', () => {
    const opts = new IndexOptionsBuilder('idx_mysql')
      .onColumns(['path'])
  ...
  test('supports MSSQL include columns', () => {
    const opts = new IndexOptionsBuilder('idx_mssql')
      .onColumns(['userId'])
