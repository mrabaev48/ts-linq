...
describe('Dialect migration SQL', () => {
  ...
  it('drop table emits DROP TABLE (all dialects)', () => {
    ...
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(diff, 'postgresql').up[0]).toBe(
      'DROP TABLE "Obsolete"'
    );
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(diff, 'mysql').up[0]).toBe(
      'DROP TABLE `Obsolete`'
    );
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(diff, 'mssql').up[0]).toBe(
      'DROP TABLE [Obsolete]'
    );
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(diff, 'sqlite').up[0]).toBe(
      'DROP TABLE Obsolete'
    );
  });
  it('ADD COLUMN NOT NULL with DEFAULT (postgres/mysql/mssql/sqlite)', () => {
    ...
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(base, 'postgresql').up[0]).toBe(
      'ALTER TABLE "Users" ADD COLUMN "active" BOOLEAN NOT NULL DEFAULT TRUE'
    );
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(base, 'mysql').up[0]).toBe(
      'ALTER TABLE `Users` ADD COLUMN `active` TINYINT(1) NOT NULL DEFAULT 1'
    );
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(base, 'mssql').up[0]).toBe(
      'ALTER TABLE [Users] ADD [active] BIT NOT NULL DEFAULT 1'
    );
    // sqlite: BOOLEAN→INTEGER, TRUE→1
    expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(base, 'sqlite').up[0]).toBe(
