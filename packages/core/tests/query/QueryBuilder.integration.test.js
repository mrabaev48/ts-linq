...
describe('QueryBuilder with Enhanced SQL Cache Integration', () => {
  ...
  describe('Enhanced Cache Integration', () => {
    ...
    test('should embed window function expressions in select list', () => {
      const options = {
        select: [
          'id',
          SqlFunctions_1.sql.rowNumber().over().partitionBy('name').orderBy('age', 'DESC').as('rn')
        ],
        where: []
      };
      ...
      const localBuilder = new QueryBuilder_1.QueryBuilder(
        exprDialect,
        mockLogger,
        'test-provider',
        enhancedCache
    ...
    test('should render basic full-text search expressions', () => {
      const options = {
        select: [
          SqlFunctions_1.sql
            .mysqlMatchAgainst(['title', 'content'], 'node js', 'boolean')
            .as('mscore'),
          SqlFunctions_1.sql
            .pgRank(
              SqlFunctions_1.sql.pgToTsVector(['title', 'content']).toString(),
              SqlFunctions_1.sql.pgWebsearchToTsQuery('node js').toString()
            )
            .as('pscore')
        ],
        where: []
    ...
    test('mysql: JSON and MATCH should be parameterized', () => {
      ...
      const local = new QueryBuilder_1.QueryBuilder(
        mysqlLikeDialect,
        mockLogger,
        'mysql',
        enhancedCache
    ...
    test('should expire cached queries after TTL', async () => {
      ...
      const shortTtlBuilder = new QueryBuilder_1.QueryBuilder(
        mockDialect,
        mockLogger,
        'test-provider',
        shortTtlCache
    ...
    test('should handle cache eviction when max size exceeded', () => {
      ...
      const smallCacheBuilder = new QueryBuilder_1.QueryBuilder(
        mockDialect,
        mockLogger,
        'test-provider',
        smallCache
  ...
  describe('Default Enhanced Cache', () => {
    test('should use enhanced cache by default when no external cache provided', () => {
      ...
      const builderWithoutCache = new QueryBuilder_1.QueryBuilder(
        mockDialect,
        mockLogger,
        'test-provider'
      );
  ...
  describe('Cache Performance Monitoring', () => {
    ...
    test('should identify cache optimization opportunities', () => {
      ...
      const testBuilder = new QueryBuilder_1.QueryBuilder(
        mockDialect,
        undefined,
        'test',
        highEvictionCache
