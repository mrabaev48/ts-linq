...
const EnhancedSqlCache_1 = require("../../src/query/EnhancedSqlCache");
const SqlFunctions_1 = require("../../src/query/SqlFunctions");
...
describe('QueryBuilder with Enhanced SQL Cache Integration', () => {
    ...
    beforeEach(() => {
        ...
        core_1.MetadataStorage.addEntity(TestUser, 'test_users');
        const cols = [
        ...
        cols.forEach((c) => core_1.MetadataStorage.addColumn(TestUser, c));
        core_1.MetadataStorage.addPrimaryKey(TestUser, 'id');
        enhancedCache = new EnhancedSqlCache_1.EnhancedSqlCache({
            maxSize: 100,
            defaultTtl: 60000,
            enableMetrics: true,
            enableLru: true
        ...
        queryBuilder = new QueryBuilder_1.QueryBuilder(mockDialect, mockLogger, 'test-provider', enhancedCache);
    });
    afterEach(() => {
    ...
    describe('Enhanced Cache Integration', () => {
        ...
        test('should provide optimization insights through QueryBuilder', () => {
            ...
            expect(insights.topAccessedEntries[0].accessCount).toBeGreaterThan(insights.topAccessedEntries[1].accessCount);
        });
        test('should embed window function expressions in select list', () => {
            const options = {
                select: ['id', SqlFunctions_1.sql.rowNumber().over().partitionBy('name').orderBy('age', 'DESC').as('rn')],
                where: []
            };
            ...
            const localBuilder = new QueryBuilder_1.QueryBuilder(exprDialect, mockLogger, 'test-provider', enhancedCache);
        ...
        test('should allow CTE name override via options.from (dialect usage)', () => {
            ...
            const local = new QueryBuilder_1.QueryBuilder(exprDialect, mockLogger, 'test', enhancedCache);
            const res = local.generateSql(TestUser, options);
        ...
        test('should render JSON helpers in select list (dialect responsibility)', () => {
            const options = {
                select: [
                    SqlFunctions_1.sql.jsonExtract('data', '$.user.name').as('u_name'),
                    SqlFunctions_1.sql.jsonContains('tags', 'admin').as('has_admin')
                ],
                where: []
            };
            const jsonDialect = {
            ...
            const local = new QueryBuilder_1.QueryBuilder(jsonDialect, mockLogger, 'test', enhancedCache);
            const res = local.generateSql(TestUser, options);
        ...
        test('should render basic full-text search expressions', () => {
            const options = {
                select: [
                    SqlFunctions_1.sql.mysqlMatchAgainst(['title', 'content'], 'node js', 'boolean').as('mscore'),
                    SqlFunctions_1.sql
                        .pgRank(SqlFunctions_1.sql.pgToTsVector(['title', 'content']).toString(), SqlFunctions_1.sql.pgWebsearchToTsQuery('node js').toString())
                        .as('pscore')
                ],
                where: []
            };
            ...
            const local = new QueryBuilder_1.QueryBuilder(ftsDialect, mockLogger, 'test', enhancedCache);
            const res = local.generateSql(TestUser, options);
        ...
        test('mysql: JSON and MATCH should be parameterized', () => {
            const options = {
                select: [
                    SqlFunctions_1.sql.jsonExtract('payload', '$.a.b').as('val'),
                    SqlFunctions_1.sql.mysqlMatchAgainst(['title', 'body'], 'golang', 'boolean').as('score')
                ],
                where: []
            };
            // Use a MySQL-like stub dialect that respects selectParams
            const mysqlLikeDialect = {
                buildSelect: (_ctor, opts) => ({
                    query: `SELECT ${opts.select.join(', ')} FROM test_users`,
                    parameters: opts.selectParams ?? []
                })
            };
            const local = new QueryBuilder_1.QueryBuilder(mysqlLikeDialect, mockLogger, 'mysql', enhancedCache);
            const res = local.generateSql(TestUser, options);
        ...
        test('should expire cached queries after TTL', async () => {
            const shortTtlCache = new EnhancedSqlCache_1.EnhancedSqlCache({
            ...
            const shortTtlBuilder = new QueryBuilder_1.QueryBuilder(mockDialect, mockLogger, 'test-provider', shortTtlCache);
            const options = { select: ['*'], where: [] };
        ...
        test('should handle cache eviction when max size exceeded', () => {
            const smallCache = new EnhancedSqlCache_1.EnhancedSqlCache({
            ...
            const smallCacheBuilder = new QueryBuilder_1.QueryBuilder(mockDialect, mockLogger, 'test-provider', smallCache);
    ...
    describe('Default Enhanced Cache', () => {
        test('should use enhanced cache by default when no external cache provided', () => {
            ...
            const builderWithoutCache = new QueryBuilder_1.QueryBuilder(mockDialect, mockLogger, 'test-provider');
            const options = { select: ['*'], where: [] };
    ...
    describe('Cache Performance Monitoring', () => {
        ...
        test('should identify cache optimization opportunities', () => {
            ...
            const highEvictionCache = new EnhancedSqlCache_1.EnhancedSqlCache({
                maxSize: 5,
                enableMetrics: true
            });
            // Create a mock dialect for testing
            ...
            const testBuilder = new QueryBuilder_1.QueryBuilder(mockDialect, undefined, 'test', highEvictionCache);
...
//# sourceMappingURL=QueryBuilder.integration.test.js.map