...
describe('Rename table/column', () => {
    test('table rename per dialect', () => {
        ...
        expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(diff, 'postgresql').up[0]).toContain('RENAME TO');
        expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(diff, 'mysql').up[0]).toContain('RENAME TABLE');
        expect((0, DialectMigrationSql_1.generateMigrationFromDiff)(diff, 'mssql').up[0]).toContain('sp_rename');
    });
