...
class FakeCounter {
    ...
    labels(lbl) {
        return {
            inc: (v) => {
                if (this.name.endsWith('db_cache_hits_total'))
                    hits.push({ lbl, v: v ?? 1 });
                if (this.name.endsWith('db_cache_misses_total'))
                    misses.push({ lbl, v: v ?? 1 });
            }
        };
    }
}
...
describe('PrometheusSqlLogger', () => {
    it('increments counters and observes duration when client provided', () => {
        const logger = new prometheus_sql_logger_1.PrometheusSqlLogger('test', { client: fakeClient, prefix: 'tsl_' });
