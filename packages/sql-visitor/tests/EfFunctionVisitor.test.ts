...
describe('EfFunctionVisitor — PostgreSQL', () => {
  ...
  it('dateDiffDay — date arithmetic', () => {
    ...
    const result = visitor.visit(node, makeCtx({ inputParameters: [new Date('2024-01-01')] }));
  ...
  it('with parameterRef resolves from inputParameters', () => {
    ...
    const result = visitor.visit(node, makeCtx({ inputParameters: ['%john%'] }));
    expect(result.condition).toBe('name LIKE ?');
  ...
  it('positional parameters ($1, $2)', () => {
    ...
    const result = visitor.visit(node, makeCtx({ state }));
...
describe('EfFunctionVisitor — MySQL', () => {
  ...
  it('random — RAND()', () => {
    expect(visitor.visit(efNode('random'), makeCtx()).condition).toBe('RAND()');
  });

  it('dateDiffDay — DATEDIFF()', () => {
    const node = efNode('dateDiffDay', prop('createdAt'), { type: 'parameterRef', index: 0 });
    const result = visitor.visit(node, makeCtx({ inputParameters: [new Date()] }));
  ...
  it('iLike throws UNSUPPORTED_FUNCTION on MySQL', () => {
    ...
    expect(() => visitor.visit(node, makeCtx())).toThrow('iLike');
  });

  it('stDev — STDDEV()', () => {
    expect(visitor.visit(efNode('stDev', prop('val')), makeCtx()).condition).toBe('STDDEV(val)');
  });

  it('variance — VARIANCE()', () => {
    expect(visitor.visit(efNode('variance', prop('val')), makeCtx()).condition).toBe(
      'VARIANCE(val)'
    );
  });
});
...
describe('EfFunctionVisitor — MSSQL', () => {
  ...
  it('like — column LIKE ?', () => {
    ...
    expect(visitor.visit(node, makeCtx()).condition).toBe('name LIKE ?');
  });

  it('random — ABS(CHECKSUM(NEWID()))', () => {
    expect(visitor.visit(efNode('random'), makeCtx()).condition).toBe('ABS(CHECKSUM(NEWID()))');
  });

  it('dateDiffDay — DATEDIFF(DAY, ...)', () => {
    const node = efNode('dateDiffDay', prop('createdAt'), { type: 'parameterRef', index: 0 });
    expect(visitor.visit(node, makeCtx({ inputParameters: [new Date()] })).condition).toContain(
      'DATEDIFF(DAY,'
    );
  });
  ...
  it('greatest — SELECT MAX from VALUES', () => {
    ...
    expect(visitor.visit(node, makeCtx()).condition).toContain('SELECT MAX(v)');
  });

  it('least — SELECT MIN from VALUES', () => {
    const node = efNode('least', prop('a'), prop('b'));
    expect(visitor.visit(node, makeCtx()).condition).toContain('SELECT MIN(v)');
  });

  it('stDev — STDEV()', () => {
    expect(visitor.visit(efNode('stDev', prop('val')), makeCtx()).condition).toBe('STDEV(val)');
  });

  it('variance — VAR()', () => {
    expect(visitor.visit(efNode('variance', prop('val')), makeCtx()).condition).toBe('VAR(val)');
  });
