...
## Architecture & OOP
- Introduce SQL Dialect Strategy (SQLiteStrategy, PostgresStrategy, MySQLStrategy) and inject into QueryBuilder
- Predicate → AST + Visitor: parse where/select/join into AST; generate SQL via Visitor per dialect
- Specification pattern for reusable filters (composable specs with toSql()/test())
- Query Object: keep intent (AST/options) separate from execution/mapping services
- Template Method in providers for connection/execute/map hooks
...
## Performance
- Anti N+1: batch one-to-many loading using IN queries per relation (per request)
...
## API & DX
- Validate include names against metadata; descriptive errors
...
- Implement include batching (one-to-many via IN) in EntityLoader
- Introduce ComparisonOperator enum and refactor parser to use it internally
- Add SQL generation cache in QueryBuilder (simple Map with option hash)
- Validate includes at runtime against metadata and throw clear errors
- Add unit tests for new enum + parser paths, and integration tests for batched includes
